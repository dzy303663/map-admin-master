<template>
	<div class="content-page uk-body" v-loading="loading">
		<div class="calendar" >
			...
			<table class="sign_tab" border="0px" cellpadding="0px" cellspacing="0px">
				...
				<tbody>
					<tr v-for="item in dateArr" v-if="contains(item)">
						<td v-for="data in item" :class="{ 'cur_day': data == date, 'ui-state-default': true }"
						    v-if="isCheck(data)">
						</td>
...
export default {
	...
	methods: {
		getCheck(){
			this.loading = true;
			this.axios.get('/api/user/checkin',{params: {month: this.month}}).then(({data}) => {
				this.checkin = data.res_data;
				setTimeout(() => {
					this.loading = false;	
				}, 200);
			})
...
.calendar button.month-less {
  ...
  top: 10px;
  cursor: pointer;
  background: url(~assets/images/checkin/left-icon.png) no-repeat left -60px;
}
.calendar button.month-add {
  right: 0px;
  cursor: pointer;
  top: 10px;
  background: url(~assets/images/checkin/right-icon.png) no-repeat left -60px;
}
.calendar .sign_tab {
  width: 100%;
...
.calendar .ui-state-default {
  background: url(~assets/images/checkin/checkin.png) no-repeat center center;
